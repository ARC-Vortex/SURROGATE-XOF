...
/*	Vue.component('bar', {
  ...
  `
    ...
    Vue.component('tooltip-wrapper', {
        props: ['html', 'placement', 'posX', 'posY', 'sizeX', 'sizeY'],
        data: () => ({ v: false, s: false, h: false, t: 0, l: 0, last: 0, i: false }),
        methods: {
            update() {
                this.h = this.v = true;
                this.$nextTick(() => {
                    const r = (this.$refs.trig.firstElementChild || this.$refs.trig).getBoundingClientRect();
                    const z = (window.tmp?.displayZoom) || 1, [V, H] = (this.placement || 'top-center').split('-');
                    const x = H === 'left' ? r.left : H === 'right' ? r.right : r.left + r.width / 2;
                    const y = V === 'top' ? r.top : V === 'bottom' ? r.bottom : r.top + r.height / 2;
                    this.l = x + (parseFloat(this.posX) || 0) * z;
                    this.t = y + (parseFloat(this.posY) || 0) * z;
                    this.i = true;
                });
            },
            k(e) {
                const n = Date.now();
                if (e.shiftKey && e.key.toLowerCase() === "x") this.s = this.v = false;
                if (e.shiftKey && this.v && this.h && (n - this.last > 300)) { this.s = !this.s; this.last = n; }
            }
        },
        mounted() { window.addEventListener('keydown', this.k, true) },
        beforeDestroy() { window.removeEventListener('keydown', this.k, true) },
        template: `
          <div style="display: contents;">
            <span ref="trig" @mouseenter="update" @mouseleave="h=false; !s && (v=false)" style="display: contents;"><slot/></span>
            <transition name="tooltip-fade">
              <div v-if="v && html" class="tooltip-box" :class="{'sticky-active':s}" @mouseenter="h=true" @mouseleave="h=false; !s && (v=false)"
                   :style="{ position:'fixed', top:t+'px', left:l+'px', width:sizeX?sizeX+'px':'auto', height:sizeY?sizeY+'px':'auto', zIndex:s?100001:100000, pointerEvents:s?'auto':'none', display:i?'block':'none',
               transform: (function(p){ 
                 const [V,H]=p.split('-'); 
                 return 'translate('+(H=='left'?'0%':H=='right'?'-100%':'-50%')+','+(V=='top'?'-100%':V=='bottom'?'0%':'-50%')+')' 
               })(placement||'top-center') }" v-html="html"/>
            </transition>
          </div>`
    });
