var systemComponents = {
		...
		template: `
			<div class="upgRow">
				<div v-for="tab in Object.keys(data)">
					...
					'background' : tmp[layer].extraColor,
					'color' : tmp[layer].color,
					'border-bottom' : tmp[layer].color
					  
					},(subtabShouldNotify(layer, name, tab) ? {'box-shadow': 'var(--hqProperty2a), 0 0 20px '  + (data[tab].glowColor || defaultGlow)} : {}), tmp[layer].componentStyles['tab-button'], data[tab].buttonStyle]"
							v-on:click="function(){player.subtabs[layer][name] = tab; updateTabFormats(); needCanvasUpdate = true;}">{{tab}}
						<tooltip v-if="data[tab].tooltip" :text="data[tab].tooltip"></tooltip>
					</button>
				</div>
			</div>
	...
	'layer-tab': {
		...
		<div v-if="tmp[layer].staticFormat">
        <column :layer="layer" :data="tmp[layer].staticFormat" :key="this.$vnode.key + '-static'"></column>
    </div>
		</div>
			`
	},
