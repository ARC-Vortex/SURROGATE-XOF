...
function applyOfflineProgress(realDiff) {
    if (!player.offTime?.remain) return realDiff;

    const maxOffline = modInfo.offlineLimit * 3600;
    player.offTime.remain = Math.min(player.offTime.remain, maxOffline);

    const offlineDiff = Math.max(player.offTime.remain / 10, realDiff * 3);
    player.offTime.remain -= offlineDiff;

    const totalDiff = realDiff + offlineDiff;
...
function gameTick(now = performance.now()) {
    if (!rafRunning) return;

    const hasGameState = player !== undefined && tmp !== undefined;
    if (!hasGameState || (tmp.gameEnded && !player.keepGoing)) {
        requestAnimationFrame(gameTick);
        return;
    }

    let realDiff = (now - lastTime) / 1000;
    lastTime = now;

    realDiff = applyOfflineProgress(realDiff);
    ...
    tmp.scrolled = (document.getElementById("treeTab")?.scrollTop ?? 0) > 30;
    ...
    gameLoop(diff);
    fixNaNs();
    adjustPopupTime(realDiff);
    updateParticles(realDiff);
